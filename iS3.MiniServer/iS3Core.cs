...
namespace iS3.MiniServer
{
    ...
    public sealed class iS3TerritoryHandle : iS3AreaHandle
    {
        ...
        public iS3DomainHandle GetDomainHandle(string NameOrID)
        {
            ...
            bool exist = DomainHandles.Any(c => c.ID == NameOrID);
            if (exist)
            {
                result = DomainHandles.First(c => c.ID == NameOrID);
            }
            else
            {
    ...
    public static class MiniServer
    {
            ...
            iS3MainDbContext ctx)
        {
            ...
            bool exist = await ctx.TerritoryHandles.AnyAsync(c => c.ID == nameOrID);
            if (exist)
            {
                result = await ctx.TerritoryHandles.FirstAsync(c => c.ID == nameOrID);
                return result;
            }
        ...
        {
            string ID = territory.TerritoryHandle.ID;
